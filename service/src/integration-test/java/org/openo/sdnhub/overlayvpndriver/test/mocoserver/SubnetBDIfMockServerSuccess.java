...
public class SubnetBDIfMockServerSuccess extends MocoHttpServer {

    private static final Logger LOGGER = LoggerFactory.getLogger(SubnetBDIfMockServerSuccess.class);

    public SubnetBDIfMockServerSuccess() {
        super();
    }
    ...
    private class SubnetSuccessResponseHandler extends MocoResponseHandler {
        ...
        public void processRequestandResponse(HttpRquestResponse httpObject) {
            ...
            ACResponse<List<ACBDInfo>> inputInstanceList = JsonUtil.fromJson(httpResponse.getData(), new TypeReference<ACResponse<List<ACBDInfo>>>() {});

            ResultRsp<ACResponse<List<ACBDInfo>>> newResult = new ResultRsp<ACResponse<List<ACBDInfo>>>(ErrorCode.OVERLAYVPN_SUCCESS);
