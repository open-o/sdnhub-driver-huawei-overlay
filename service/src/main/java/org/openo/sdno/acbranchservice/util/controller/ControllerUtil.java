...
package org.openo.sdno.acbranchservice.util.controller;
...
public class ControllerUtil<T> {
    ...
    public List<T> checkRsp(HTTPReturnMessage httpMsg) throws ServiceException {
        ...
        AcBranchResponse<List<T>> acBranchResponse =
                JsonUtil.fromJson(body, new TypeReference<AcBranchResponse<List<T>>>() {});
        if(!acBranchResponse.isSucess()) {
            String errMsg = "checkRsp AC return error, errCode: " + acBranchResponse.getErrcode() + ", errMsg: "
                    + acBranchResponse.getErrmsg();
            LOGGER.error(errMsg);
            SvcExcptUtil.throwInnerErrSvcExptionWithInfo(ErrorCode.OVERLAYVPN_FAILED, errMsg, null, null, null);
        }

        return acBranchResponse.getData();
    }
}
